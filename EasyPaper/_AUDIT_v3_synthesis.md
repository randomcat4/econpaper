...
**Source import + integration is feasible.** The two hard preconditions from the brief do not block:

1. **GitHub upstream exists.** `the original EasyPaper source repository` is public. `master`, `HEAD`, and tag `v0.2.4` all resolve to `8797d4bf9cd8680f4da2d8b322533364d602b82d`. PyPI 0.2.4 was uploaded minutes after that tag and the local `econpaper/EasyPaper` tree matches the PyPI sdist shape.
2. **ccproxy is compatible for the main path.** EasyPaper centralizes OpenAI-compatible text model calls through `LLMClient -> AsyncOpenAI(**kwargs)`, and agents pass explicit `base_url` from `ModelConfig`. econpaper must inject an EasyPaper config with `base_url=http://127.0.0.1:<ccproxy_port>/codex/v1` and `api_key=ccproxy-oauth`; env-only routing is not enough.

**Recommended path:** import the source baseline at `v0.2.4` / `8797d4bf...`, keep upstream remote for manual merges, and build a thin econpaper wrapper around `EasyPaper.generate_stream()` rather than merging EasyPaper's internal agents into econpaper YAML one by one.
...
- Six closed PRs, all by `original maintainer`.
- Recent commits are effectively one-author (`Yuwei Yan` / `original maintainer`).
...
- Use a hard source-import baseline at `v0.2.4`, document upstream SHA, and treat upstream merges as manual review events because bus factor is low.
...
1. econpaper starts/ensures ccproxy with existing `maybe_start_ccproxy()`.
2. econpaper constructs an EasyPaper `AppConfig` in memory or writes a temporary config YAML.
...
5. Progress events are translated into econpaper channel/thinking events at the tool/subagent layer.
...
- Therefore, do not deeply merge internal StateGraphs into econpaper's `langgraph_dev` manifest for Tier 1.5. Add a wrapper graph/subagent only if async execution is needed.
...
- econpaper/skill4econ/econ analysis produces real figure artifacts.
...
- Tier 3 `econ_analysis_agent` is not a source-import decision blocker, but it is the right provenance path for trustworthy econ figures.
...
- About 46 files are plausible direct compatibility-regression candidates, including pure/source-level tests and tests that already mock LLM/http/Docling behavior.
...
- Source-import baseline + source provenance + pin documentation: **0.5 day**
...
The source-import decision should not be blocked by missing upstream or ccproxy incompatibility; both are manageable. The imported baseline should be conservative: pin `v0.2.4`, wrap the SDK boundary, inject ccproxy config explicitly, and keep EasyPaper's internal LangGraph machinery boxed behind `generate_stream()`.
