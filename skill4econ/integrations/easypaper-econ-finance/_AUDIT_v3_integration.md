...
- **LangGraph verdict:** EasyPaper already uses LangGraph internally. Integration should treat EasyPaper as a graph-based orchestrator to wrap, not as a set of simple prompt files to merge into econpaper subagents one by one.
...
```powershell
git ls-remote --heads --tags the original EasyPaper source repository
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/repos/the original EasyPaper source project'
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/repos/the original EasyPaper source project/tags?per_page=100'
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/repos/the original EasyPaper source project/releases?per_page=100'
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/search/issues?q=repo:the original EasyPaper source project+type:issue&per_page=10'
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/search/issues?q=repo:the original EasyPaper source project+type:pr&per_page=10'
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/repos/the original EasyPaper source project/commits?per_page=20'
Invoke-RestMethod -Headers @{ 'User-Agent'='codex-audit' } -Uri 'https://api.github.com/repos/the original EasyPaper source project/contributors?per_page=100'
...
- Repo: the original EasyPaper source repository
- GitHub API repo record: https://api.github.com/repos/the original EasyPaper source project
- Tags API: https://api.github.com/repos/the original EasyPaper source project/tags?per_page=100
...
`the original EasyPaper source project` exists and is public. GitHub API reported:

- `full_name`: `the original EasyPaper source project`
- `default_branch`: `master`
- `created_at`: `2026-02-08T14:22:41Z`
- `updated_at`: `2026-05-24T04:47:20Z`
- `pushed_at`: `2026-05-24T04:49:16Z`
- `stargazers_count`: 4
- `source_repo_forks_count`: 4
- `open_issues_count`: 0
- `archived`: false
- `disabled`: false

Rendered GitHub page also showed 146 commits, 11 tags, 4 stars, 4 source-repo forks, 0 open issues, and 0 open PRs on 2026-06-10.
...
The local source at `econpaper/EasyPaper` has no `.git`, includes `PKG-INFO` and `easypaper.egg-info`, and has file timestamps around `2026-05-24 12:50` local time, matching the PyPI upload time in UTC+8. It looks like an extracted source distribution, not a clone.
...
- PR #6, `v0.2.4`, by `original maintainer`, created `2026-05-24T04:47:07Z`, closed `2026-05-24T04:47:16Z`
- PR #5, `Prevent public setup drift from breaking richer SDK installs`, by `original maintainer`, created `2026-05-23T17:14:48Z`, closed `2026-05-23T17:14:56Z`
- PRs #1-#4 are also closed and by `original maintainer`
...
```text
original maintainer, contributions=30, original maintainer profile
```

The latest 20 commits API results all have commit author `Yuwei Yan`; GitHub login was either `original maintainer` for merge commits or `null` for direct commits. Examples:

- `8797d4bf9cd8680f4da2d8b322533364d602b82d`, `2026-05-24T04:47:15Z`, `Merge pull request #6 from original maintainer/commercial`
...
## econpaper Integration Surface

### ccproxy

Relevant files:

- `econpaper/ccproxy_manager.py`
- `econpaper/config/settings.py`
...
- econpaper's settings layer can also propagate configured API keys/base URLs into process env, but only if those env vars are not already set (`settings.py:523` through `settings.py:528`).
...
- `econpaper/langgraph_dev/manager.py`
- `econpaper/langgraph_dev/langgraph.json`
- `econpaper/langgraph_dev/graphs.py`

Integration facts:

- econpaper runs `langgraph dev` as a subprocess for async subagents (`manager.py:1`, `manager.py:329`).
- Default dev server base URL is `http://127.0.0.1:6174` (`manager.py:48`, `manager.py:51`).
- The packaged manifest is `econpaper/langgraph_dev/langgraph.json` (`manager.py:312`, `manager.py:321`).
...
If EasyPaper is exposed as an econpaper async subagent, adding YAML is not enough: a graph binding and `langgraph.json` entry are also required.

### Channels

The three channel files inspected were `econpaper/channels/standalone.py`, `econpaper/channels/telegram/channel.py`, and `econpaper/channels/telegram/serve.py`.
...
EasyPaper progress events should be converted to econpaper thinking/outbound updates at the agent/tool layer so Telegram and future channels receive the same behavior.

### Subagent YAML Shape

Relevant files:

- `econpaper/subagents/*.yaml`
- `econpaper/utils.py`
- `econpaper/subagents/_factory.py`
- `econpaper/econpaper.py`
...
- `_maybe_swap_async_subagents()` replaces `_async` subagents with remote `AsyncSubAgent` specs only when `enable_async_subagents` and `langgraph dev` are available (`econpaper.py:244` through `econpaper.py:323`).
...
- `econpaper/EasyPaper/.git` is absent.
- `econpaper/EasyPaper/PKG-INFO` and `easypaper.egg-info/` are present.
...
econpaper already has newer LangGraph/DeepAgents dependencies:
...
EasyPaper's `langgraph>=0.0.26` has no upper bound, so dependency resolution is probably compatible, but it must be smoke-tested against econpaper's resolved LangGraph 1.x API.
...
- `api_base`: no matches in `econpaper/EasyPaper` or `econpaper`.
- `OPENAI_BASE_URL` / `ANTHROPIC_BASE_URL`: no EasyPaper source matches. These exist in econpaper ccproxy/config code only.
...
- If EasyPaper did not use LangGraph, the simple path would be a thin econpaper tool that calls a plain SDK method.
- Because EasyPaper already uses LangGraph, the integration should wrap EasyPaper's SDK orchestration boundary (`EasyPaper.generate()` / `generate_stream()`) rather than trying to translate every EasyPaper agent into econpaper YAML subagents.
- For econpaper async execution, the clean path is an `easypaper-agent` wrapper graph/subagent that calls `EasyPaper.generate_stream()` and forwards events. Deeply merging EasyPaper's internal StateGraphs into econpaper's `langgraph_dev` manifest would couple two orchestration systems and increase maintenance cost.

## Recommended Integration Plan

1. Import the original EasyPaper source project baseline at `v0.2.4` / `8797d4bf9cd8680f4da2d8b322533364d602b82d`.
2. Vendor the imported source initially with a version lock; keep an upstream remote for periodic manual merges.
3. Add an econpaper wrapper tool/subagent that:
   ...
   - converts EasyPaper progress events into econpaper channel/thinking events.
...
5. If the wrapper is async-subagent backed, add:
   - `econpaper/subagents/easypaper.yaml`
...
Proceed with a source-import-and-integrate path. Do not downgrade to PyPI-only vendoring. The repo and PyPI artifact are sufficiently aligned for a hard source-import baseline, but the upstream is one-author/low-activity, so integration should be pinned and tested rather than tracking floating `master`.
