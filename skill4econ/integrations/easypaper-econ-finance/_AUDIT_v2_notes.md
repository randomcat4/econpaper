...
`ask` 路由到 memory/planner/reviewer 三个 handler,**是用户跨 agent 提问的统一入口**,对 IM 聊天式交互(econpaper 主线场景)有直接价值。`show_markdown/show_json_data` 是 UI 流式输出工具。
...
`pyproject.toml` 注册了 `integration` marker(`pyproject.toml:50`),但代码里几乎没用过。**没有现成的方式按 marker 区分线下/线上测试**;独立项目化后需要自己加 marker 或维护 ignore list。
...
- 工时仍是 L2(planner/section 改动量未变),但**架构对齐了 upstream 已经画出的方向**,source import 长期维护成本更低。
...
**对 Tier 3 的意义**:
- 这些图必须来自**外部 econ_analysis_agent**(econpaper 主线),作为 `metadata.figures: List[FigureSpec]` 用户输入注入。
- `FigureSpec` 模型支持 `auto_generate=False` + 文件路径方式 — 这是合法的注入点。
- 5/31 推荐"新增 econ_analysis_agent 输出 artifacts 喂 writer"完全正确,**但理由不是'easypaper 缺画图能力',而是 easypaper 主动拒绝这条路径**。独立项目化后即使我们想从内部生成,也要先解锁这个角色白名单。
...
5/31 audit §8 修正了用户最初的"没有可见 GitHub repo"假设,声称 `the original EasyPaper source project` 公开存在(146 commits / 11 tags / 4 stars)。

v2 无法在本次离线审查中复核:
- 源码 `PKG-INFO / pyproject.toml / README.md / setup.cfg` 没有 GitHub URL
- `rg original maintainer|github.com` 在 src 里**零命中**
- 5/31 当时是怎么确认的没写入 audit(可能用了 WebSearch/WebFetch)

**建议**: 在签 source import 决策前用 WebFetch 复查 `github.com/the original EasyPaper source project` 是否仍存在、tag 是否与 PyPI 0.2.4 对齐,作为 upstream tracking 策略的前置条件。本次 audit 不替换 5/31 的结论,但标 "未独立复核"。

---

## E. 给 econpaper 主线的修订执行建议
...
理由: §C.2,接口设计原本就指向这里;减少 source import 长期发散。
...
Tier 1 ≈ 1 周,Tier 2 ≈ 3-5 周,Tier 3 ≈ 2-3 个月(走 econpaper `econ_analysis_agent` 外部 artifact 路径,不在 easypaper 独立项目内做)。

### E.4 Top 3 风险 — 重排序

5/31 列了 3 风险。v2 排序:

1. **`figure_supplementation` 角色白名单硬阻断 data visualization**(C.3)— 比"缺画图能力"更严,独立项目化后要么解锁要么完全走外部图注入。
...
- §4 Tier 3 段:补一句"easypaper figure 系统主动拒绝 data visualization"(C.3),从根本上把 Tier 3 排除在 easypaper 独立项目范围外。
...
- `langgraph_dev` 在 easypaper 里是否使用(econpaper 主线在用)— 没 grep,留待后续。
...
- 中文 / GBK 支持现状 — easypaper 是否有显式 encoding 处理,本次未扫,但 econpaper 已经踩过 ccproxy/OAuth 那一坑,可能复用同一套 fix。
