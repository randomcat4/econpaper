...
def test_spatial_se_comparison_outputs_cutoff_grid(tmp_path: Path) -> None:
    ...
    assert "SPATIAL_HAC_UNIFORM_KERNEL" in _codes(result)
    assert result["is_full_conley"] is False
    table = pd.read_csv(tmp_path / "se" / "tables" / "spatial_se_comparison.csv")
    assert {"spatial_hac_uniform_cutoff", "cluster:city:numpy"} & set(table["se_type"].astype(str))
    hac_rows = table[table["se_type"].astype(str) == "spatial_hac_uniform_cutoff"]
    assert set(hac_rows["kernel"].astype(str)) == {"uniform"}
