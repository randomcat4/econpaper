# Skill: evidence_lookup_rules

## Purpose

Specify source lookup order for stable methods and volatile policy, regulatory, standards, and data-source facts.
...
- Check repo capabilities and tests before external method claims.
- Use classic econometrics papers and official package docs for stable methodology.
- For ESG, carbon accounting, CBAM, MRV, climate-risk regulation, and data APIs, query official/latest sources at use time.
- Do not let literature review substitute for code diagnostics.
- Put source uncertainty in `uncertainty_notes`.
...
```yaml
skill_name: evidence_lookup_rules
user_question_summary: "Check whether CBAM coverage can be hardcoded."
research_domain: evidence_lookup
uncertainty_notes:
  - "CBAM coverage is volatile; query official/latest sources at use time."
next_code_actions: []
```
