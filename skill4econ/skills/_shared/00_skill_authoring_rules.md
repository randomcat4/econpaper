# Skill: skill_authoring_rules

## Purpose

Define the mandatory structure, output contract, and safety boundary for all skill4econ skill files.
...
- Use the fixed section template exactly for every skill file except the top-level README.
- Require YAML or JSON outputs and include `Forbidden claims`, `Handoff to code`, and `Handoff from code artifacts`.
- Do not use TWFE as the default entry for staggered environmental policies.
- Use plain candidate language until code validates the spec and artifacts.
- Name the artifact or rule that allows any strong claim.
...
```yaml
skill_name: skill_authoring_rules
user_question_summary: "Audit a draft skill."
research_domain: authoring
forbidden_claims:
  - do_not_omit_claim_gate
  - do_not_call_a_draft_spec_validated
next_code_actions:
  - run_skill_file_checklist
```
