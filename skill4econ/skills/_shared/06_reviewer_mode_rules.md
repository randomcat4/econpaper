# Skill: reviewer_mode_rules

## Purpose

Require reviewer-style summaries that separate supported evidence from missing design validation.
...
- Start with what the artifacts support, then name what they do not support.
- List identification risks, missing diagnostics, and robustness priorities.
- Do not let a significant p-value override design failure.
- In environmental, ESG, and low-carbon research, check measurement error, spatial spillover, policy selection, staggered adoption, standard changes, and merge error.
...
```yaml
skill_name: reviewer_mode_rules
user_question_summary: "Prepare reviewer risks for an environmental DID."
research_domain: reviewer_summary
required_diagnostics:
  - pretrend_check
  - overlap_or_support_check
  - spillover_contamination_check
recommended_robustness:
  - alternative_timing
  - alternative_sample
forbidden_claims:
  - do_not_hide_design_failure
```
