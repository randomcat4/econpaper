# Skill: spec_drafting_rules

## Purpose

Define how to turn a research question into a draft spec without pretending the spec is validated.
...
- Mark every uncertain field that needs user confirmation.
- List unit, time, outcome, treatment/exposure, covariates, fixed effects, cluster, backend, and artifacts.
- Do not emit raw Stata or R commands as the spec.
- Require metadata for W matrices, CRS, coordinates, emissions units, carbon boundaries, and sample construction.
- Send the draft to code for validation before estimation.
...
```yaml
skill_name: spec_drafting_rules
user_question_summary: "Draft a spec for low-carbon pilot DID."
research_domain: spec_draft
research_brief:
  unit: firm
  time_frequency: year
...
uncertainty_notes:
  - "Policy timing, firm-city mapping, and cluster level need confirmation."
next_code_actions:
  - validate_spec
```
